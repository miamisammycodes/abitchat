...
- **D-06 (KNOWN CONSEQUENCE — message bodies):** Encrypting `conversations`/`messages` content with a non-deterministic cast **breaks**: DEC-14's keyword `LIKE` fallback tier in RAG retrieval, conversation search/filtering, conversation analytics, lead-scoring/quality tooling that reads message text, and admin conversation-review UX (decrypt-per-row, no search). pgvector cosine still works (embeddings are derived vectors, not raw text). This is an **accepted, deliberate** decision. The planner MUST emit a downstream-impact note and the Phase 21 (Analytics & Notifications) plan MUST account for encrypted transcripts. Planner should assess whether retrieval/search needs a redesign sub-task or an explicit "degraded" acceptance this phase.
