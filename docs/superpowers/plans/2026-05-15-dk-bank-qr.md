...
## Task 12.5: Update `submitPayment` to merge into awaiting_payment row

**Why:** Spec requires that if a merchant generated a DK QR (creating an `awaiting_payment` row) and then goes back and fills out the manual form for the same plan, we UPDATE the existing row rather than creating a duplicate. Otherwise admin sees two transactions for one merchant's intent.

**Files:**
- Modify: `app/Http/Controllers/Client/BillingController.php`
- Test: `tests/Feature/BillingTest.php` (append new test method)

- [ ] **Step 1: Write the failing test**

In `tests/Feature/BillingTest.php`, add this test method to the existing `BillingTest` class:
...
```bash
php artisan test --filter=test_submit_payment_merges_into_awaiting_dk_qr_row
```

Expected: FAIL — assertion count is 2 (the original + a new one created by submitPayment).

- [ ] **Step 3: Update `submitPayment` to merge or insert**

In `app/Http/Controllers/Client/BillingController.php`, replace the `try { Transaction::create(...) }` block inside `submitPayment` with:
...
Wrap the whole block in `DB::transaction(function () use (...) { ... });` if it isn't already, to make the lockForUpdate meaningful. The existing controller already imports `DB`; if not, add `use Illuminate\Support\Facades\DB;`.
...
- [ ] **Step 5: Run the full BillingTest to ensure no regression in the original submitPayment flow**
...
```bash
git add app/Http/Controllers/Client/BillingController.php tests/Feature/BillingTest.php
git commit -m "feat(billing): submitPayment merges into existing awaiting_payment row from DK QR"
```
...
**Pre-implementation check:** Review the `docs/superpowers/notes/2026-05-15-dk-task-0-results.md` Probe 6 outcome. If Probe 6 showed DK's `transaction_date` parameter does NOT strictly bind (i.e. tomorrow-date query succeeded on a today-payment), the two-candidate loop is unnecessary — simplify to a single call before committing. The default below assumes strict binding (worst case).
...
```bash
cat resources/js/Pages/Client/Billing/Subscribe.vue
```

Identify (a) which grid container wraps the Plan Summary card and the manual form (likely `<div class="grid grid-cols-1 md:grid-cols-3 gap-6">` but verify before editing), and (b) where the page header ("Subscribe to {{ plan.name }}") sits — the DK QR card should be inserted between the header and that grid.

- [ ] **Step 2: Add the DK QR card above the existing form grid**
...
2. Inside the `<template>`, find the grid container you identified in Step 1 (the one wrapping the Plan Summary + form). Insert IMMEDIATELY ABOVE that grid (and below the Pending Transaction Warning Alert, if present):
...
- Data model → Tasks 3 (migration + nullable transaction_number), 4 (model fields), 11 (approveAndActivate) ✓
- Service layer → Tasks 5–11 ✓
- Failure handling → Tasks 9 (5001 retry), 15 (DkQrGenerationException), 16-17 (status guards), 20 (controller tests + rate-limit), 27 (smoke) ✓
...
- Rate limiter → Task 13 + Task 20 rate-limit test ✓
- submitPayment merge logic (spec data-model section) → Task 12.5 ✓
